...
public class SalesController {
    ...
    private final static String SALES_LINK = "/sales";

    @GetMapping(value="/sales")
    public String allAccountPost(@RequestParam(value="page", defaultValue="0") int page, Model model, Principal principal){
        postService.AllPost(page, "영업부 게시판", SALES_LINK, false, PostType.NORMAL, "회계", model);
        staffInfoService.getInfo(model, principal);
        model.addAttribute("backLink", "/");
        return "/post/post";
    }

    @GetMapping(value="/sales/write")
    public String writeEditor(Model model){
        postService.createPostPage("영업부 게시글 작성", SALES_LINK, model);
        return "/post/post_write";
    }

    @PostMapping(value="/sales/write/create")
    public String createSalesPost(PostDto postDto, Principal principal, RedirectAttributes attributes){
        postService.createPost(principal, postDto, PostType.NORMAL, "영업");
        attributes.addFlashAttribute("alertData", "게시글 작성이 완료되었습니다.");
        return "redirect:/sales";
    }

    @GetMapping(value="/sales/postId={postId}")
    public String detailedSalesPost(@PathVariable("postId") Long postId, Model model, Principal principal){
        postService.readPost(postId, "영업부 게시글", SALES_LINK, model, principal);
        return "/post/post_content";
    }

    @GetMapping(value="/sales/postId={postId}/update")
    public String updateEditor(@PathVariable("postId") Long postId, Model model){
        postService.updatePostPage(postId, "영업부 게시글 편집", SALES_LINK, model);
        return "/post/post_write";
    }

    @PostMapping(value="/sales/postId={postId}/update")
    public String updateSalesPost(@PathVariable("postId") Long postId, PostDto postDto, RedirectAttributes attributes){
        Post post = postService.updatePost(postId, postDto);
        attributes.addFlashAttribute("alertData", ("수정 완료되었습니다. 제목 : " + post.getTitle()));
        return ("redirect:/sales/postId=" + postId);
    }

    @GetMapping(value="/sales/postId={postId}/delete")
    public String deleteSalesPost(@PathVariable("postId") Long postId, RedirectAttributes attributes){
    ...
    @PostMapping(value="/sales/postId={postId}/comment")
    public String createComment(@PathVariable("postId") Long postId, CommentDto commentDto, Principal principal){
        ...
        return "redirect:/sales/" + postId;
    }

    @GetMapping(value="/sales/postId={postId}/comment/commentId={commentId}/create")
    public String createReplyPage(@PathVariable("postId") Long postId, @PathVariable("commentId") Long commentId, Model model, Principal principal){
        postService.createReplyPage(postId, commentId, "영업부 게시글", SALES_LINK, model, principal);
        return "/post/post_content";
    }

    @PostMapping(value="/sales/postId={postId}/comment/commentId={commentId}/create")
    ...
    @GetMapping(value="/sales/postId={postId}/comment/commentId={commentId}/update")
    public String updateCommentPage(@PathVariable("postId") Long postId, @PathVariable("commentId") Long commentId, Model model){
        postService.updateCommentPage(postId, commentId, "영업부 게시글", SALES_LINK, model);
        return "/post/post_content";
    }
    @PostMapping(value="/sales/postId={postId}/comment/commentId={commentId}/update")
    ...
    @GetMapping(value="/sales/postId={postId}/comment/commentId={commentId}/replyId={replyId}/update")
    public String updateReplyPage(@PathVariable("postId") Long postId, @PathVariable("commentId") Long commentId, @PathVariable("replyId") Long replyId, Model model){
        postService.updateReplyPage(postId, commentId, replyId, "영업부 게시글", SALES_LINK, model);
        return "/post/post_content";
    }
    @PostMapping(value="/sales/postId={postId}/comment/commentId={commentId}/replyId={replyId}/update")
    ...
    @GetMapping(value="/sales/postId={postId}/comment/commentId={commentId}/delete")
    ...
    @GetMapping(value="/sales/postId={postId}/comment/commentId={commentId}/replyId={replyId}/delete")
